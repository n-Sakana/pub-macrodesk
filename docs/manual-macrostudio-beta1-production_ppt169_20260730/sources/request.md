添付ファイル source-code.md は、Excel ブック 申請データ検証.xlsm の VBA コード全文です
（8 モジュール、合計 484 行。省略はありません）。
...
【対象モジュール】※ 0 行のモジュールは元から空です
  ...
  - AppController （標準モジュール, 103 行）
  - SystemInfo （標準モジュール, 157 行）
  - TimerUtils （標準モジュール, 14 行）
  - WindowUtils （標準モジュール, 210 行）
...
'@MACROSTUDIO a19914fe-3f78-4492-bb89-e434000b2155 SUMMARY BEGIN
（何をどう直したか。モジュール名を挙げて 3〜6 行程度。
　マクロを書かない人にも分かる言葉で）
'@MACROSTUDIO a19914fe-3f78-4492-bb89-e434000b2155 SUMMARY END

続けて、モジュールごとに次の行ではさみます。

'@MACROSTUDIO a19914fe-3f78-4492-bb89-e434000b2155 BEGIN <種類> <モジュール名>
（そのモジュールの全文）
'@MACROSTUDIO a19914fe-3f78-4492-bb89-e434000b2155 END <種類> <モジュール名>

すべてのモジュールを書き終えたら、最後に次の 1 行を置いてください。

'@MACROSTUDIO a19914fe-3f78-4492-bb89-e434000b2155 COMPLETE <モジュールの数>
...
- a19914fe-3f78-4492-bb89-e434000b2155 は今回の依頼の番号です。書き換えたり省略したりしないでください。
