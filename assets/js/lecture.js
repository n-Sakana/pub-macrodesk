(function (global) {
  ...
  var contentByKey = {
    "L1-1": {
      title: "改修するブックを添付",
      body: [
        "対応形式は .xlsm / .xlam / .xlsb / .xls です。",
        "ウィンドウのどこへドラッグしても、［ファイルを選ぶ］でも指定できます。",
        "Excel で開いたままのブックも、そのまま添付できます。"
      ].join("\n")
    },
    "L1-2": {
      title: "読み込み完了",
    ...
    "L2-2": {
      title: "依頼を準備する",
      body: [
        "［依頼を準備する］を押すと、ブックと同じフォルダにコード全文ファイルを作ります。",
        "同時に、依頼文だけをクリップボードへコピーします。",
        "エクスプローラーが開き、そのファイルが選択されます。"
      ].join("\n")
    },
    "L2-3": {
      title: "チャット AI へ渡す",
      body: [
        "チャット AI に依頼文を貼り付けます（Ctrl+V）。",
        "選択されたコードファイルを添付して送信してください。",
    ...
    "L3-8": {
      title: "貼り付けた内容を手動修正",
      body: [
        "右の欄で、貼り付けたコードを直接直せます。",
        "余分な行の削除や写し間違いなど、軽微な手直しに使います。",
        "大きな変更は②に戻って依頼を言い直してください。",
        "直したら［修正を反映］で差分を確認します。"
      ].join("\n")
    },
    "L4-1": {
      title: "新しいブックを作成",
  ...
  var errorContentByCode = {
    "E-ATTACH-02": {
      title: "ファイルを読み取れません",
      body: [
        "ブックの場所とアクセス権を確認してください。",
        "移動やリネームをした場合は、もう一度選び直してください。"
      ].join("\n")
    },
    "E-ATTACH-03": {
      title: "マクロのあるブックを確認",
    ...
    "E-GEN-02": {
      ...
      body: [
        ...
        "templates\\request-template.txt を UTF-8 で保存し、" +
          "差し込み（{{REQUEST_TEXT}} 必須）を確認してください。"
      ].join("\n")
    },
    "E-GEN-03": {
      title: "依頼文をコピーし直す",
      body: [
        "依頼文をクリップボードへコピーできませんでした。",
        "コードファイルは作成済みです。",
        "［依頼文をもう一度コピー］を押してやり直してください。"
      ].join("\n")
    },
    "E-PASTE-01": {
      title: "コードをコピーし直す",
